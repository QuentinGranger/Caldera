...
export function parseEmailSnapshot(value: unknown): EmailSnapshot {
  ...
  if (row.pickupPoint !== undefined && row.pickupPoint !== null) {
    if (!row.pickupPoint || typeof row.pickupPoint !== 'object')
      throw new Error('Snapshot email invalide.');
    const pickup = row.pickupPoint as Record<string, unknown>;
    if (
      typeof pickup.pointId !== 'string' ||
      typeof pickup.name !== 'string' ||
      !Array.isArray(pickup.address) ||
      pickup.address.some((line) => typeof line !== 'string')
    )
      throw new Error('Snapshot email invalide.');
  }
